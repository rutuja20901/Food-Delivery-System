...
//		RestaurantResponse res = restTemplate.getForObject(url, RestaurantResponse.class);
		...
		ResponseEntity<RestaurantResponse> response = restTemplate.exchange(url, HttpMethod.GET, entity,
...
//			MenuResponse menuResponse = restTemplate.getForObject(menuUrl, MenuResponse.class);
			...
			ResponseEntity<MenuResponse> response1 = restTemplate.exchange(menuUrl, HttpMethod.GET, entity1,
					MenuResponse.class);
			...
			BigDecimal subTotal = res1.getPrice().multiply(BigDecimal.valueOf(itemRequest.getQuantity()));
		...
		NotificationEvent event = new NotificationEvent(savedOrder.getUserId(), savedOrder.getId(), "Order Placed",
	...
	public String cancelOrder(Long id) {
		...
		NotificationEvent event = new NotificationEvent(order.getUserId(), "Order Rejected",
	...
	public OrderResponse acceptByRestaurant(Long id) {
		...
		Order savedOrder = orderRepository.save(order);
		NotificationEvent event = new NotificationEvent(order.getUserId(), "Order Accepted",
	...
	public OrderResponse outForDelivery(Long id) {
		...
		NotificationEvent event = new NotificationEvent(order.getUserId(), "Order out for delivery",
	...
	public OrderResponse orderDelivered(Long id) {
		...
		Order saved = orderRepository.save(order);
		NotificationEvent event = new NotificationEvent(order.getUserId(), "Order delivered", "Your order is delivery");
		producer.sendNotification(event);
		return mapToResponse(saved);
	}
	 ...
	 * Convert OrderItem entities to DTOs.
	 * Map order details and item details.
